...
#1. 원형 vs AABB: 모서리 부분에서 어떤 차이? 사각형의 오브젝트 에서는 AABB가 모서리 충돌 감지를 더 정확하게 하고 원형일 경우에는 모서리를 감지 못하는 경우가 있다.
#2. AABB vs OBB: 회전 시 어떤 차이? AABB는 오브젝트의 회전에 관계없이 충돌 감지가 정적으로 유지되고 OBB는 오브젝트의 회전에 따라 충돌 감지도 같이 회전한다.
#3. 어떤 방식이 내 게임에 더 적합할까? 단순한 플랫포머 게임에서는 AABB를 사용하는 것이 적합할 것 같다.